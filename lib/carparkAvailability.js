...
const MAX_PAGES = 8; // safety cap (~4000 records - observed real usage is ~6 pages/~3000 records nationwide)
...
async function fetchCarParkAvailability() {
  ...
  // Pages are fetched in parallel (not sequentially) - this dataset is
  // large enough (~6 pages / ~3000 records observed in production) that
  // fetching one page at a time took 11+ seconds end to end. Firing all
  // MAX_PAGES requests at once brings that down to roughly the time of the
  // single slowest request. The tradeoff: pages past the real end of the
  // dataset just come back empty (harmless, cheap) instead of being skipped.
  const pagePromises = Array.from({ length: MAX_PAGES }, (_, page) => {
    const url = `${CARPARK_AVAILABILITY_URL}?$skip=${page * PAGE_SIZE}`;
    return fetch(url, { headers: { AccountKey: accountKey, accept: "application/json" } })
      .then(async (res) => {
        if (!res.ok) {
        ...
        const records = json?.value ?? json?.Value ?? json?.results ?? json?.Results ?? (Array.isArray(json) ? json : null);
      ...
      .catch((err) => {
        console.warn(`LTA CarParkAvailability fetch error (page ${page}):`, err.message);
        return [];
      });
  });

  const allRecords = (await Promise.all(pagePromises)).flat();
